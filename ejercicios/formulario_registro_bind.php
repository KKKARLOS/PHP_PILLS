...
if (isset($_POST["enviar"])){
    ...
    else
    {
          ...
          mysqli_stmt_bind_param($result, "sssssss", $codigo,$seccion,$nombre,$precio,$fecha,$importado,$paisorigen);
         ...
         $contador=mysqli_stmt_affected_rows($result); //Para insert, update, delete
          ...
          if ($contador>0) echo "Datos grabados correctamente";
...
if (isset($_POST["borrar"])){
    ...
    mysqli_set_charset($conexion,"utf8");
    //query
    $consulta="DELETE FROM PRODUCTOS  WHERE CÓDIGOARTÍCULO=?";
    //echo $consulta;
    $result=mysqli_prepare($conexion,$consulta);
    if (!$result){
    ...
    //parametros
    // Si el tipo de dato del parámetro fuera un entero pondríamos una "i" y si  fuera decimal una "d"
          mysqli_stmt_bind_param($result, "s", $codigo);
          mysqli_stmt_execute($result);
          mysqli_stmt_store_result($result); 
          $contador=mysqli_stmt_affected_rows($result); //Para insert, update, delete
            //$contador=mysqli_stmt_num_rows($result);   //Para select
               
          if ($contador>0) echo "Datos eliminados correctamente";
          else echo "Error en grabación.";
    }
    // Cerrar la conexión
    mysqli_close($conexion);
